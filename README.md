...
После `python manage.py migrate` в Admin появится включённый поиск «Домклик Уфа: 2–3 комнаты» с заданными фильтрами. Выдача рендерится в Chromium; fast-проверка читает первую страницу и не деактивирует объявления:
...
Как и Avito, плановые Docker-запуски DomClick используют новый профиль Chromium,
но сохраняют общий cooldown и диагностику. Для ручной проверки доступен
`--headed --wait-for-captcha 300`: он только ждёт действий пользователя в уже
открытом окне и не перезагружает страницу.
...
случайная задержка до пяти минут). Включите его после
установки unit-файлов:

```bash
sudo systemctl enable --now home-hunter-domclick-fast.timer
```
...
Расписание включается для `home-hunter-avito-fast.timer` и
`home-hunter-domclick-fast.timer`; ЦИАН настраивается отдельно.
